...
function obdelajSlike(socket) {
  socket.on('slike', function(sporocilo) {;
    if (sporocilo.kanal) {
    socket.broadcast.to(sporocilo.kanal).emit('slike', {
 ...
 function obdelajDregljaj(socket) {
   socket.on('dregljaj', function (dregljaj) {
     if (dregljaj.vzdevek) {
      ...
      if (socketIdNaslovnika == socket.id) {
       io.sockets.sockets[socket.id].emit('sporocilo', {
         besedilo: "Dregljaja samemu sebi ne morete poslati"
       }); 
      } else if (socketIdNaslovnika) {
        io.sockets.sockets[socketIdNaslovnika].emit('dregljaj', { dregljaj: true });
       io.sockets.sockets[socket.id].emit('sporocilo', {
         besedilo: "Dregljaj za '" + dregljaj.vzdevek + "'"
       });
       io.sockets.sockets[socketIdNaslovnika].emit('sporocilo', {
         besedilo: "Dregljaj od '" + vzdevkiGledeNaSocket[socket.id] + "'"
       });
       } else {
        io.sockets.sockets[socket.id].emit('sporocilo', {
...
function obdelajVideo(socket) {
  socket.on('video', function(sporocilo) {;
    if (sporocilo.kanal) {
    socket.broadcast.to(sporocilo.kanal).emit('video', {
        besedilo: sporocilo.besedilo
      });
    }
  });
